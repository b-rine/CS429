...
cache_line_t *get_line(cache_t *cache, uword_t addr) {
    // does this memory address exist in cache?
    // if yes, then return pointer to cache line
    size_t s = _log(cache->C / (cache->A * cache->B));
    size_t b = _log(cache->B);

    // extract bits with a mask
    uword_t setInd = (addr >> b) & ((1 << s) - 1);
    uword_t t = addr >> (b + s); 

    // iterating through cache lines
    for (int i = 0; i < cache->A; i++) {
        // check if valid and exists
        if (cache->sets[setInd].lines[i].valid && cache->sets[setInd].lines[i].tag == t) {
            return &(cache->sets[setInd].lines[i]);
        }
    }
...
cache_line_t *select_line(cache_t *cache, uword_t addr) {
    ...
    // extract bits with a mask
    uword_t setInd = (addr >> b) & ((1 << s) - 1);
    uword_t minLRU = 0xFFFFFFFFFFFFFFFF;
    cache_line_t *pointerLRU = NULL;

    // iterating through cache lines
    for (int i = 0; i < cache->A; i++) {
        // check if valid and exists
        if (!cache->sets[setInd].lines[i].valid) {
            return &(cache->sets[setInd].lines[i]);
        }
    }

    // finding the LRU cache
    for (int i = 0; i < cache->A; i++) {
        if (cache->sets[setInd].lines[i].lru < minLRU) {
            minLRU = cache->sets[setInd].lines[i].lru;
            pointerLRU = &(cache->sets[setInd].lines[i]);
        }
    }
...
bool check_hit(cache_t *cache, uword_t addr, operation_t operation) {
    ...
    // updating LRU for all cache lines?
...
evicted_line_t *handle_miss(cache_t *cache, uword_t addr, operation_t operation, byte_t *incoming_data) {
    ...
    if (line->valid) {
        ...
        evicted_line->addr = (line->tag << (s+b)) | (setInd << b);
...
void get_word_cache(cache_t *cache, uword_t addr, word_t *dest) {
    cache_line_t *line = get_line(cache, addr);
    unsigned int offset = addr & ((1 << _log(cache->B)) - 1);
    byte_t *dst = (byte_t *) dest;
    for (int i = 0; i < 8; i++) {
        dst[i] = line->data[offset + i];
    }
...
void set_word_cache(cache_t *cache, uword_t addr, word_t val) {
    cache_line_t *line = get_line(cache, addr);
    unsigned int offset = addr & ((1 << _log(cache->B)) - 1);
    byte_t *val_ptr = (byte_t *) &val;
    for (int i = 0; i < 8; i++) {
        line->data[offset + i] = val_ptr[i];
    }
}
