...
comb_logic_t execute_instr(x_instr_impl_t *in, m_instr_impl_t *out) {
        ...
        // Select alu val b and perform alu operation
        uint64_t val_b = in->X_sigs.valb_sel ? in->val_b : in->val_imm;
        alu(in->val_a, val_b, in->val_hw, in->ALU_op, in->X_sigs.set_CC, in->cond, &(out->val_ex), &(out->cond_holds), &(guest.proc->NZCV));
