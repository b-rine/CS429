...
bool check_mispred_branch_hazard(opcode_t X_opcode, bool X_condval) {
    if ((X_opcode == OP_B_COND || X_opcode == OP_CBZ || X_opcode == OP_CBNZ) && X_condval == false) {
        return true;
    }
...
bool check_load_use_hazard(opcode_t D_opcode, uint8_t D_src1, uint8_t D_src2,
    ...
    // Specifically back-to-back load use
    if (X_opcode == OP_LDUR && (X_dst == D_src1 || X_dst == D_src2) && (D_opcode != OP_MOVZ &&
        D_opcode != OP_ADRP && D_opcode != OP_B && D_opcode != OP_B_COND && D_opcode != OP_BL)) {
        return true;
    }
...
comb_logic_t handle_hazards(opcode_t D_opcode, uint8_t D_src1, uint8_t D_src2, uint64_t D_val_a, 
    ...
    // Determine which hazard(s)
    bool ret_hazard = check_ret_hazard(D_opcode);
    bool branch_hazard = check_mispred_branch_hazard(X_opcode, X_condval);
    bool load_hazard = check_load_use_hazard(D_opcode, D_src1, D_src2, X_opcode, X_dst);

    // Ret hazard
    if (ret_hazard || D_opcode == OP_BR || D_opcode == OP_BLR) {
    ...
    // Mispredicted branch hazard
    } else if (branch_hazard) {
        pipe_control_stage(S_FETCH, false, false);
        pipe_control_stage(S_DECODE, true, false);
        pipe_control_stage(S_EXECUTE, true, false);
    }
