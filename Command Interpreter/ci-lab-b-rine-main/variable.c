...
void put(char *id, node_t *nptr) {
    ...
    entry_t *curr = var_table->entries[hash_function(id)];
    if (curr == NULL) {
        var_table->entries[hash_function(id)] = eptr;
        return;
    } else if (strcmp(curr->id, eptr->id) == 0) {
        eptr->next = curr->next;
        var_table->entries[hash_function(id)] = eptr;
        delete_entry(curr);
        return;
    ...
    while (curr->next != NULL) {
        if (strcmp(curr->next->id, eptr->id) == 0) {
            eptr->next = curr->next->next;
            delete_entry(curr->next);
            curr->next = eptr;
            return;
        }
        curr = curr->next;
...
entry_t* get(char* id) {
    ...
    entry_t *eptr = var_table->entries[hash_function(id)];

    while (strcmp(eptr->id, id) != 0) {
        eptr = eptr->next;
    }
